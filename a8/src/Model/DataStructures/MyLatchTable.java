...
public class MyLatchTable implements MyILatchTable{
    ...
    public void put(int key, int value) throws EvaluationException {
        synchronized (this) {
            if (!latchTable.containsKey(key)) {
                ...
                throw new EvaluationException(String.format("Latch table already contains the key %d!", key));
            }
        }
    }

    @Override
    public int get(int key) throws EvaluationException {
        synchronized (this) {
            if (latchTable.containsKey(key)) {
                return latchTable.get(key);
            } else {
                throw new EvaluationException(String.format("Latch table doesn't contain the key %d!", key));
            }
        }
    }
    ...
    @Override
    public void update(int key, int value) throws EvaluationException {
        synchronized (this) {
            if (latchTable.containsKey(key)) {
